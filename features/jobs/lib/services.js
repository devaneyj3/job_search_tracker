...
import { daysFromNow } from "@/features/shared/lib/utils";
 ...
 * @param {string} [jobData.skill1] - First skill
 * @param {string} [jobData.skill2] - Second skill
...
export async function updateJobApplication(jobId, data, originalEmail) {
	...
	if (data.contactEmail != originalEmail) {
		data.initialContactEmailSent = true;
		data.initialContactDate = new Date();
		data.secondContactEmailSent = false;
		data.secondContactDate = daysFromNow(new Date.now(), "5");
